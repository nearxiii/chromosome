...
class SentpcrExport implements FromCollection, WithHeadings , WithMapping , ShouldAutoSize , WithCustomStartCell , WithEvents , WithColumnFormatting 
{
    /**
    ...
    */
    ...
    {
        $startDate = request()->input('from_date_sent') ;
        $endDate   = request()->input('to_date_sent') ;
        return Sentpcr::whereBetween('created_at', [ $startDate, $endDate ] )
            ->get();
    }
    public function map($sentpcrs) : array {
        return [
            \PhpOffice\PhpSpreadsheet\Shared\Date::dateTimeToExcel($sentpcrs->created_at),
            $sentpcrs->lab_no,
            $sentpcrs->pt_name,
            $sentpcrs->pt_add,
            $sentpcrs->sample_type,
            $sentpcrs->sample_quelity,
            $sentpcrs->sample_con,
            $sentpcrs->sample_clot,
            $sentpcrs->pcr_result,
            $sentpcrs->dna_conc,
            \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($sentpcrs->dna_date),
            $sentpcrs->dna_staff,
            $sentpcrs->pcr_conc,
            \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($sentpcrs->pcr_date),
            $sentpcrs->pcr_staff,
            $sentpcrs->frag_conc,
            \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($sentpcrs->frag_date),
            $sentpcrs->frag_staff,
            $sentpcrs->dilute_fac,
            \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($sentpcrs->analyz_date),
            $sentpcrs->analyz_staff,
            \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($sentpcrs->virified_date),
            $sentpcrs->virified_staff,
            \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($sentpcrs->email_date),
            $sentpcrs->email_staff,
            $sentpcrs->remark,
